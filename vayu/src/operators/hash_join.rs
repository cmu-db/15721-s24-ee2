...
use crate::operators::operator_result_type::{SinkResultType,OperatorResultType};
use crate::pipeline::{IntermediateOperator, PhysicalOperator, Sink};
use std::hash::Hasher;
...
{
	fn name(&self) -> String {
        String::from("hash_build")
    }
}

impl Sink for HashJoinBuildOperator
{
    fn sink(&mut self, batch : &Arc<RecordBatch>, results: &mut Vec<RecordBatch>) -> SinkResultType {
