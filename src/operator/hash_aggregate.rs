...
impl HashAggregateOperator {
    pub fn new(aggregate_expr : Vec<Arc<dyn AggregateExpr>>, group_by_expr: Vec<(Arc<dyn PhysicalExpr>,String)>) -> Self {
        Self {
...
impl Sink for HashAggregateOperator {
    fn sink(&mut self, input: &Arc<RecordBatch>) -> SinkResultType {
        ...
        let group_by_keys= self.group_by_expr.iter().map(
            |(f, _name)| f.evaluate(input).unwrap().into_array(input.num_rows()).unwrap()).collect::<Vec<_>>();
        ...
        for row_idx in 0..num_rows {
            ...
            for col_idx in 0..group_by_keys.len() {
                match group_by_keys[col_idx].data_type() {
                    DataType::Int32 => {
                        let col: &Int32Array = group_by_keys[col_idx].as_any().downcast_ref().unwrap();
                        let value = col.value(row_idx);
                        hasher.write_i32(value);
                    }
                    DataType::Utf8 =>{
                        let col: &GenericByteArray<Utf8Type> = group_by_keys[col_idx].as_any().downcast_ref().unwrap();
                        let value = col.value(row_idx);
            ...
            if !self.hash_table.contains_key(&group_by_key) {
                ...
                self.hash_table.get_mut(&group_by_key).unwrap().push(hash_value);
            }
        }
    ...
    fn finalize(&mut self) {
        ...
        for (expr, _) in &self.group_by_expr {
            ...
            let agg_col = compute::take(&new_col.into_array(combined.num_rows()).unwrap(), &indices, None).unwrap();
            columns.push(agg_col);
        }

        for agg_expr in &self.aggregate_expr {
            let input_exprs = agg_expr.expressions();
            let all_input_cols = input_exprs.iter().map(|e| {
                e.evaluate(&combined).and_then(|v| v.into_array(combined.num_rows()))
            ...
            for (_, group) in &self.hash_table {
                ...
                let input_cols = all_input_cols.iter().map(|c| {
                    compute::take(c, &group, None)
                }).collect::<Result<Vec<_>,_>>().unwrap();
            ...
            let output_col_ref = output_col.iter().map(|a| {
                a.as_ref()
            }).collect::<Vec<_>>();
        ...
        for(expr , name ) in &self.group_by_expr{
            fields.push(Field::new(name, expr.data_type(&self.originals[0].schema()).unwrap(), false));
        }
        for expr in &self.aggregate_expr{
            fields.push(expr.field().unwrap());
        }
        let out_schema = Schema::new(fields);
        self.aggregated_data.data = Some(RecordBatch::try_new(Arc::new(out_schema), columns).unwrap());
    }
}
