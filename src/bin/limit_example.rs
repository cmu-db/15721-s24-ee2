...
fn main() {
    ...
    //create limit sink
    let sink: Option<Box<dyn Sink>> = Some(Box::new(LimitOperator::new(10)));
    ...
    let limit_res = pipeline2.sink_operator.take().unwrap();
    let limit_res= limit_res.as_any().downcast_ref::<LimitOperator>().unwrap();
    let limit_data = &limit_res.data;
    //TODO make sure that the result is not empty
    //panics! if the result is empty
    let limit_data = compute::concat_batches(
        &limit_data.originals[0].schema(), limit_data.originals.iter()).unwrap();

    pretty::print_batches(std::slice::from_ref(&limit_data)).unwrap();
}
