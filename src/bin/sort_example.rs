...
fn main() {
    ...
    let sink : Option<Box<dyn Sink>> = Some(Box::new(HashJoinBuildOperator::new(v, Arc::new(schema1.clone()))));
    ...
    let probe : Box<dyn IntermediateOperator> = Box::new(HashJoinProbeOperator::new(v, Arc::new(schema2.clone()), build_data));
    ...
    let sort_opeartor = sort_operator.as_any().downcast_ref::<SortOperator>().unwrap();
