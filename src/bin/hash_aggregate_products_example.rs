...
fn main(){
    ...
    let aggregate_expr = create_aggregate_expr(&AggregateFunction::Sum, false, &[aggregate_expr.clone()], &[], &schema, "total_price").unwrap();

    let v = vec![(group_by_expr,String::from("fruit"))];
    //create the join build operator
    let sink : Option<Box<dyn Sink>> = Some(Box::new(HashAggregateOperator::new(vec![aggregate_expr], v)));
    ...
    let build  = build.as_any().downcast_ref::<HashAggregateOperator>().unwrap();

    let grouped_data = &build.aggregated_data;
    let _ = pretty::print_batches(std::slice::from_ref(grouped_data.data.as_ref().clone().unwrap()));
